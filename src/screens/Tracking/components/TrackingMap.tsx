...
function makeIcon(status: TrackedTrip['status']) {
  if (!L) return null;
...
export function TrackingMap({ trips, selectedId, onSelect, onReport, onResolve }: Props) {
  ...
  const mapRef       = useRef<ReturnType<typeof L.prototype.map> | null>(null);
  const markersRef   = useRef<Map<string, ReturnType<typeof L.prototype.marker>>>(new Map());
  ...
  useEffect(() => {
    let mounted = true;
    ensureLeaflet().then(() => {
      if (!mounted || !containerRef.current || mapRef.current || !L) return;
  ...
  useEffect(() => {
    ...
    trips.forEach((t) => {
      ...
      if (existing) {
        ...
        const marker = L.marker([t.position.lat, t.position.lng], { icon })
          .addTo(map)
