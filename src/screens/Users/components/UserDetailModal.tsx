...
import type { PlatformUser, PlatformUserStatus, PlatformUserVerification } from '../../../models/user.model';
...
export function UserDetailModal({ user, detailLoading, onClose, onApproveKyc, onRejectKyc, loadingId }: Props) {
  const [confirmReject, setConfirmReject] = useState(false);
  const [lightbox, setLightbox] = useState<{ images: LightboxEntry[]; index: number } | null>(null);

  const isPending = user.verification === 'En attente';
  const busy      = loadingId === user.id;

  const allImages: LightboxEntry[] = [
    user.selfies?.front  && { url: user.selfies.front,  label: 'Selfie de face' },
    user.selfies?.left   && { url: user.selfies.left,   label: 'Selfie gauche' },
    user.selfies?.right  && { url: user.selfies.right,  label: 'Selfie droite' },
    user.idCard?.front   && { url: user.idCard.front,   label: "Carte d'identité (recto)" },
    user.idCard?.back    && { url: user.idCard.back,    label: "Carte d'identité (verso)" },
  ...
  const hasKyc = !!(user.selfies?.front || user.idCard?.front || user.idCard?.back);
      ...
      <DetailModal title="Détail Utilisateur" onClose={onClose} accentColor="#2563EB">
        ...
        <div className="detail-hero" style={{ background: 'rgba(37,99,235,0.06)' }}>
          <HeroAvatar
            ...
            name={user.name}
            onClick={user.selfies?.front ? () => openLightbox(user.selfies!.front!, 'Selfie de face') : undefined}
          />
          <div>
            ...
            <div className="detail-hero__badge" style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              ...
              {detailLoading && <span style={{ fontSize: 11, color: '#9CA3AF' }}>Chargement…</span>}
            </div>
          </div>
        </div>
        ...
        <DetailSection title="Photos & Documents KYC">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <PhotoCard url={user.selfies?.front} label="De face" icon={User} onOpen={url => openLightbox(url, 'Selfie de face')} />
            <PhotoCard url={user.selfies?.left}  label="Gauche"  icon={User} onOpen={url => openLightbox(url, 'Selfie gauche')} />
            <PhotoCard url={user.selfies?.right} label="Droite"  icon={User} onOpen={url => openLightbox(url, 'Selfie droite')} />
          </div>

          <SubTitle>Carte d'identité / Passeport</SubTitle>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <PhotoCard url={user.idCard?.front} label="Recto" icon={CreditCard} onOpen={url => openLightbox(url, "Carte d'identité (recto)")} />
            <PhotoCard url={user.idCard?.back}  label="Verso" icon={CreditCard} onOpen={url => openLightbox(url, "Carte d'identité (verso)")} />
          </div>
        ...
        {isPending && onApproveKyc && onRejectKyc && (
          <div style={{ display: 'flex', gap: 10, marginTop: 8, borderTop: '1px solid #F3F4F6', paddingTop: 16 }}>
            {!confirmReject ? (
              ...
              <div style={{ flex: 1, background: '#FEF2F2', border: '1.5px solid #FCA5A5', borderRadius: 8, padding: '10px 12px' }}>
                ...
                <p style={{ fontSize: 11, color: '#6B7280', marginBottom: 10 }}>
                  L'utilisateur devra soumettre à nouveau ses documents.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => { onRejectKyc(user.id); onClose(); }} style={{
            ...
            <button type="button" disabled={busy} onClick={() => { onApproveKyc(user.id); onClose(); }} style={{
              ...
              Approuver KYC → Vérifié
            </button>
          </div>
        )}

        {/* Badge "Vérifié" */}
        {user.verification === 'Vérifié' && (
          <div style={{
