...
const STATUS_CFG: Record<RefundStatus, { label: string; color: string; bg: string; icon: typeof Clock }> = {
  ...
  remboursé:  { label: 'Remboursé',   color: '#00A86B', bg: '#DCFCE7', icon: CheckCircle  },
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: 380, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 20 }}>
          <div style={{ background: '#F0FDF4', borderRadius: 10, padding: 14, marginBottom: 16, border: '1px solid #BBF7D0' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: '#00A86B', marginTop: 4 }}>{fmt(amount)}</div>
          </div>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Méthode de remboursement</span>
            {METHODS.map((m) => (
              <label key={m} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, border: `2px solid ${method === m ? '#00A86B' : '#E5E7EB'}`, cursor: 'pointer', background: method === m ? '#F0FDF4' : '#fff' }}>
                <input type="radio" name="method" value={m} checked={method === m} onChange={() => setMethod(m)} style={{ accentColor: '#00A86B' }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: method === m ? '#00A86B' : '#374151' }}>{m}</span>
              </label>
            ))}
          </label>
        </div>
        <div style={{ padding: '14px 20px', borderTop: '1px solid #F3F4F6', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button type="button" onClick={() => onConfirm(method)} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#00A86B', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer' }}>Approuver</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: 360, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 20 }}>
          <div style={{ fontSize: 13, color: '#374151', marginBottom: 12 }}>
            Confirmez que <strong>{name}</strong> a bien reçu <strong style={{ color: '#00A86B' }}>{fmt(amount)}</strong>.
          </div>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
    ...
    <DashboardLayout title="Remboursements Passagers">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: 'Remboursements faits',value: String(summary.totalRefunded),             color: '#00A86B', bg: '#DCFCE7', icon: CheckCircle  },
          { label: 'Total remboursé',     value: fmt(summary.refundedAmount),               color: '#00A86B', bg: '#DCFCE7', icon: RefreshCw    },
        ].map((k) => (
        ...
        {loading ? (
          ...
          return (
            ...
            >
              ...
              {/* Montant */}
              <div>
                <div style={{ fontSize: 13, fontWeight: 800, color: '#00A86B' }}>{fmt(r.refundAmount)}</div>
